...
```text
    [ INPUTS ]               [ DECODER (Brain) ]             [ SYNTHESIZERS (Body) ]
   
    (Pitch)  f0  ───┐        ┌───────────────┐               ┌────────────────────┐
                    ├───▶───┤  GRU (Linear) ├───▶──┬────────▶┤ Harmonic Oscillator│──┐
    (Volume) L0  ───┘        └───────────────┘   │  (Amps)   └────────────────────┘  │
                                                 │                                   ▼
                                                 │           ┌────────────────────┐ (Sum) ──▶ [ AUDIO ]
                                                 └──────────▶┤ Noise Filter Bank  │──┘
...
*The model learns to map human expression (pitch/loudness) into technical synthesizer parameters, "performing" the guitar in real-time.*
